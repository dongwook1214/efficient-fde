...
func (c *Circuit) Define(api frontend.API) error {
	...
	// Poseidon2 permutation for BLS12-381
...
func randomFrElement() (bls12381fr.Element, error) {
	var e bls12381fr.Element
	_, err := e.SetRandom()
	return e, err
}

func samplePedersenLikeBases(size int) ([]bls12381.G1Affine, error) {
	bases := make([]bls12381.G1Affine, size)
	for i := range bases {
...
func setupDummyCPLink(M [][]bls12381.G1Affine) (cpLinkProvingKey, cpLinkVerifyingKey, error) {
	n := len(M)
	if n == 0 {
	...
	pk := cpLinkProvingKey{P: make([]bls12381.G1Affine, t)}
	for j := 0; j < t; j++ {
	...
	vk := cpLinkVerifyingKey{C: make([]bls12381.G2Affine, n)}
	for i := 0; i < n; i++ {
		var aki bls12381fr.Element
		aki.Mul(&a, &k[i])
...
func proveDummyCPLink(pk cpLinkProvingKey, x []bls12381fr.Element) (cpLinkProof, error) {
	if len(pk.P) != len(x) {
...
func verifyDummyCPLink(bench cpLinkBenchmark) error {
	...
	g1 := make([]bls12381.G1Affine, 0, n+1)
	g1 = append(g1, bench.Statement.Y...)
	g1 = append(g1, bench.Proof.Pi)

	g2 := make([]bls12381.G2Affine, 0, n+1)
...
// which equals VK = H^SK in BLS12-381 G1).
//
...
func benchmarkDummyCPLink(values []bls12381fr.Element) (cpLinkBenchmark, error) {
	m := len(values)
	...
	n := 1 + m
	t := 1 + m
	M := make([][]bls12381.G1Affine, n)
	for i := range M {
		M[i] = make([]bls12381.G1Affine, t)
		for j := range M[i] {
			M[i][j].SetInfinity()
		}
	}
...
func sampleProveInputs() (bls12381fr.Element, [N + 2]bls12381fr.Element, [N + 2]bls12381fr.Element) {
	var alpha bls12381fr.Element
	var x [N + 2]bls12381fr.Element
	var srPrime [N + 2]bls12381fr.Element
...
func lagrangeCoefficients(alpha bls12381fr.Element, points [N + 2]bls12381fr.Element) ([N + 2]bls12381fr.Element, error) {
	var coeffs [N + 2]bls12381fr.Element
	products := make([]bls12381fr.Element, len(points))
	alphaDiffs := make([]bls12381fr.Element, len(points))
	alphaPoint := -1
	...
	weights := bls12381fr.BatchInvert(products)
	alphaDiffInvs := bls12381fr.BatchInvert(alphaDiffs)

	scaledWeights := make([]bls12381fr.Element, len(points))
	partialSums := make([]bls12381fr.Element, workerCount)
...
func prove(
	num_cores int,
	skBI *big.Int,
	alpha bls12381fr.Element,
	x [N + 2]bls12381fr.Element,
	srPrime [N + 2]bls12381fr.Element,
	...
	for i := 0; i < N+2; i++ {
		...
		var hi bls12381fr.Element
		hi.SetBytes(hiBytes)

		var cti bls12381fr.Element
		cti.Add(&x[i], &hi)
		w.CT[i] = mustBig(&cti)
	...
	witness, err := frontend.NewWitness(&w, ecc.BLS12_381.ScalarField())
	if err != nil {
		log.Fatal(err)
	}
	...
	cpLinkBench, err := benchmarkDummyCPLink([]bls12381fr.Element{tempU, skFr})
	if err != nil {
		log.Fatal(err)
	}
